...
function updateQuest() {
  ...
  $('#quest-reward').textContent = profile().completed.includes(`${selectedMission}-${profile().grade}`) ? '6–15 coins per replay' : '21–30 first adventure';
}
...
function avatarMarkup(a = profile().avatar) {
  return `<div class="hero-look"><div class="block-avatar outfit-${a.outfit || 'none'} hairstyle-${a.hairstyle || 'short'}" style="--shirt:${a.shirt};--skin:${a.skin};--hair:${a.hair}"><div class="block-accessory-back ${a.back || 'none'}"></div><div class="block-hair" aria-hidden="true"></div><div class="block-head"></div><div class="block-hat ${a.hat}"></div><div class="block-accessory-face ${a.face || 'none'}"></div><div class="block-body"></div><div class="block-arm left"></div><div class="block-arm right"></div><div class="block-leg left"></div><div class="block-leg right"></div><div class="block-skirt"></div></div>${a.pet && a.pet!=='none'?`<span class="hero-companion" role="img" aria-label="${escape(a.pet)} companion">${companionArt(a.pet)}</span>`:''}</div>`;
}
function companionArt(value) {
  return `<span class="pet-art pet-${value}" aria-hidden="true"><i class="pet-tail"></i><i class="pet-body"></i><i class="pet-ears"></i><i class="pet-head"><i class="pet-eyes"></i><i class="pet-nose"></i></i><i class="pet-feet"></i></span>`;
}
function showAvatar() {
  openPanel(`<section class="panel-content">${heading('ONE OF A KIND','Hello, hero.','Your colours are always free. Find more accessories in the hero shop.')}<button class="shop-banner" id="open-shop">${icon('star')}<span><b>The Hero Shop</b><small>Companions, outfits & adventure gear</small></span><span class="coin-icon">H</span><b>${profile().coins}</b>${icon('arrow')}</button><div class="avatar-editor"><div class="avatar-display" id="avatar-preview">${avatarMarkup()}</div><div>${[['shirt','Shirt colour',COLORS.shirts],['skin','Skin tone',COLORS.skins],['hair','Hair colour',COLORS.hairs]].map(([key,label,values])=>`<div class="field-group"><span class="field-label">${label}</span><div class="swatches">${values.map((value,i)=>`<button class="swatch ${profile().avatar[key] === value ? 'selected' : ''}" style="background:${value}" data-style="${key}" data-value="${value}" aria-label="${label} ${i+1}" aria-pressed="${profile().avatar[key] === value}"></button>`).join('')}</div></div>`).join('')}<div class="field-group"><span class="field-label">Finishing touch</span><div class="hat-options">${[['none','Just my hair'],['cap','Everyday cap'],['explorer','Explorer hat']].map(([value,label])=>`<button data-style="hat" data-value="${value}" class="${profile().avatar.hat === value ? 'selected' : ''}" aria-pressed="${profile().avatar.hat === value}">${label}</button>`).join('')}</div></div></div></div><div class="panel-actions"><button class="primary-button" data-close>Ready for adventure ${icon('arrow')}</button><span class="offline-label">Your look saves automatically.</span></div></section>`);
  $('.avatar-editor').insertAdjacentHTML('beforebegin', `<div class="character-picker"><div class="field-group"><span class="field-label">Choose your character · always free</span><div class="character-options" role="group" aria-label="Character">${[['boy','Boy','short'],['girl','Girl','ponytail']].map(([value,label,hairstyle])=>`<button data-character="${value}" aria-pressed="${(profile().avatar.character || 'boy')===value}"><span class="character-portrait" aria-hidden="true">${avatarMarkup({...profile().avatar,hairstyle,hat:'none',pet:'none',back:'none',face:'none'})}</span><b>${label}</b><small>Free</small></button>`).join('')}</div></div><div class="field-group"><span class="field-label">Hairstyle · always free</span><div class="hat-options" role="group" aria-label="Hairstyle">${[['short','Short'],['bob','Bob'],['ponytail','Ponytail'],['pigtails','Pigtails']].map(([value,label])=>`<button data-style="hairstyle" data-value="${value}" class="${(profile().avatar.hairstyle || 'short')===value?'selected':''}" aria-pressed="${(profile().avatar.hairstyle || 'short')===value}">${label}</button>`).join('')}</div><p class="character-note">Mix any hairstyle, colour or outfit. Everything in the shop is for every hero.</p></div></div>`);
  content.querySelectorAll('[data-character]').forEach(button=>{button.onclick=()=>{
    Object.assign(profile().avatar,{character:button.dataset.character,hairstyle:button.dataset.character==='girl'?'ponytail':'short',hat:'none'});
    save();syncWorld();showAvatar();
  };});
...
function showShop(selectedId = 'headphones', category = 'all') {
  ...
  const categories = [['all','All'],['pet','Companions'],['outfit','Outfits'],['hat','Headwear'],['back','Gear'],['face','Accessories']];
  const pet = item.slot === 'pet';
  const visible = COSMETICS.filter(c=>category==='all'||c.slot===category);
  openPanel(`<section class="panel-content hero-shop">${heading('LITTLE REWARDS. BIG PERSONALITY.','The Hero Shop',`You have ${p.coins} Hero Coins. Play, learn, and save for something you love.`)}
    <div class="shop-showcase"><div class="shop-preview"><span class="preview-label">${pet?'MEET YOUR COMPANION':'TRYING IT ON'}</span>${avatarMarkup(draft)}<span class="preview-floor"></span><small class="preview-note">Preview only · choose below to keep</small></div>
    <div class="shop-detail"><span class="subject-tag">${categories.find(([key])=>key===item.slot)?.[1] || 'ACCESSORIES'}</span><h3>${item.name}</h3><p>${item.description}</p><span class="shop-price">${owned?`${icon('check')} ${pet?'Part of your crew':'In your wardrobe'}`:`<span class="coin-icon">H</span> ${item.cost} coins`}</span><button class="primary-button" id="buy-cosmetic" ${equipped?'disabled':''}>${equipped?(pet?'By your side':'Wearing it'):owned?(pet?'Bring along':'Wear this'):`${pet?'Adopt & bring along':'Buy & wear'} · ${item.cost}`}</button><p id="shop-feedback" class="shop-feedback" role="status">${!owned&&p.coins<item.cost?`${item.cost-p.coins} more coins to go. You’re getting there!`:pet?'One companion comes exploring with you. Your other friends stay in your collection.':'Tap an item below to try it on. Your current look stays safe until you choose.'}</p></div></div>
    <div class="shop-categories" role="group" aria-label="Shop categories">${categories.map(([key,label])=>`<button data-shop-category="${key}" aria-pressed="${category===key}">${label}</button>`).join('')}</div>
    <div class="shop-collection-title"><b>${categories.find(([key])=>key===category)?.[1] || 'All'} treasures</b><small>${visible.length} to discover</small></div>
    <div class="cosmetic-grid">${visible.map(c=>`<button class="cosmetic-card ${c.id===item.id?'selected':''}" data-cosmetic="${c.id}" aria-pressed="${c.id===item.id}"><span class="merch-stage">${c.slot==='pet'?companionArt(c.value):`<span class="merch-art merch-${c.id}" aria-hidden="true"><i></i></span>`}</span><b>${c.name}</b><small>${p.ownedCosmetics.includes(c.id)?p.avatar[c.slot]===c.value?(c.slot==='pet'?'By your side':'Wearing'):'Owned':`${c.cost} coins`}</small></button>`).join('')}</div>
    <div class="shop-footer"><button class="secondary-button" id="shop-play">Play & earn more ${icon('star')}</button><button class="text-button" id="shop-wardrobe">My colours & basic outfits</button></div><div class="wearing-tools"><span>Change your look:</span>${[['hat','No hat'],['back','No gear'],['face','No glasses'],['outfit','Basic outfit'],['pet','Rest companion']].map(([slot,label])=>`<button class="text-button" data-unequip="${slot}">${label}</button>`).join('')}</div></section>`);
  content.querySelectorAll('[data-shop-category]').forEach(button=>{button.onclick=()=>{const next=button.dataset.shopCategory;showShop(COSMETICS.find(c=>next==='all'||c.slot===next).id,next);};});
  content.querySelectorAll('[data-cosmetic]').forEach(button=>{button.onclick=()=>showShop(button.dataset.cosmetic,category);});
  $('#buy-cosmetic').onclick=()=>{
    ...
    equipCosmetic(p,item.id);save();syncWorld();chime();showShop(item.id,category);
    $('#shop-feedback').textContent=pet?`${item.name} is ready to explore by your side!`:`Looking good! Your ${item.name.toLowerCase()} is on your hero.`;
  };
  $('#shop-wardrobe').textContent='Choose my character';
  $('#shop-play').onclick=chooseMission;$('#shop-wardrobe').onclick=showAvatar;
  content.querySelectorAll('[data-unequip]').forEach(button=>{button.onclick=()=>{p.avatar[button.dataset.unequip]='none';save();syncWorld();showShop(item.id,category);$('#shop-feedback').textContent='Saved. Everything you own stays in your collection.';};});
}
...
async function bootWorld() {
  try {
    ...
    world = createWorld($('#world'),{ reducedMotion: matchMedia('(prefers-reduced-motion: reduce)').matches,
      ...
      onSelect: location => { if(panel.open || buildMode)return; if(location==='home')showHome(); else launchActivity(location); } });
