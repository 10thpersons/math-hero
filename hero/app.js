...
function avatarMarkup(a = profile().avatar) {
  return `<div class="hero-look"><div class="block-avatar outfit-${a.outfit || 'none'}" style="--shirt:${a.shirt};--skin:${a.skin};--hair:${a.hair}"><div class="block-accessory-back ${a.back || 'none'}"></div><div class="block-head"></div><div class="block-hat ${a.hat}"></div><div class="block-accessory-face ${a.face || 'none'}"></div><div class="block-body"></div><div class="block-arm left"></div><div class="block-arm right"></div><div class="block-leg left"></div><div class="block-leg right"></div></div>${a.pet && a.pet!=='none'?`<span class="hero-companion" role="img" aria-label="${escape(a.pet)} companion">${companionArt(a.pet)}</span>`:''}</div>`;
}
