export function createProblems(type, grade, rng = Math.random) {
  ...
  let pool;
  if (grade === 1) pool = Array.from({ length: 19 }, (_, i) => type === 'bridge'
    ? { type, target: i + 2, lengths: [1, 2, 3, 5] }
    : { type, target: i + 2, quantity: 1, price: i + 2 });
  if (grade === 2) pool = Array.from({length: 79}, (_, i) => i + 21).map(target => type === 'bridge'
  ...
  if (grade === 3) pool = pairs(2, 9, 2, 9).map(([a, b]) => type === 'bridge'
    ? { type, target: a * b, lengths: [a], length: a, groups: b }
    : { type, target: a * b, quantity: a, price: b });
  if (grade === 4) pool = type === 'bridge'
    ? Array.from({length: 39}, (_, i) => i + 12).map(cm => ({ type, target: cm * 10, targetCm: cm, lengths: [10, 20, 50, 100], unit: 'mm', gridStep: 10, skill: 'Convert centimetres to millimetres',
    ...
    : pairs(2, 6, 3, 19).map(([quantity, price]) => [quantity, price, quantity * price < 50 ? 50 : 150]).filter(([quantity, price, tender]) => tender - quantity * price !== quantity * price).map(([quantity, price, tender]) => ({type, quantity, price, tender, target: tender - quantity * price, denominations: [1, 5, 10, 20, 50], skill: 'Calculate change',
  ...
  if (grade === 5) pool = type === 'bridge'
    ? Array.from({length: 35}, (_, i) => i + 5).filter(target => target % 4).map(target => ({ type, target, lengths: [1, 2, 3, 4], unit: 'm', scale: 4, skill: 'Combine fractional lengths',
    ...
    : pairs(2, 6, 21, 99).map(([quantity, cents]) => [quantity, cents * 5]).map(([quantity, price]) => ({ type, quantity, price, target: quantity * price, moneyScale: 100, denominations: [5, 10, 20, 50, 100, 500, 1000], skill: 'Multiply decimal prices',
  ...
  if (grade === 6) pool = type === 'bridge'
    ? pairs(1, 4, 1, 4).flatMap(([short, long]) => [2, 3, 4].filter(repeats => (short + long) * repeats <= 20).map(repeats => [short, long, repeats])).map(([short, long, repeats]) => ({ type, target: (short + long * 2) * repeats, lengths: [1, 2], ratio: [short, long], skill: 'Build to a ratio',
    ...
    : pairs(2, 6, 2, 10).flatMap(([quantity, tens]) => [10, 20, 25, 30, 50].filter(discount => Number.isInteger(quantity * tens * 10 * (100 - discount) / 100)).map(discount => [quantity, tens * 10, discount])).map(([quantity, price, discount]) => ({ type, quantity, price, discount, target: quantity * price * (100 - discount) / 100, denominations: [1, 5, 10, 20, 50], skill: 'Calculate a percentage discount',
  ...
  if (grade <= 2) {
    ...
    for (const problem of originals) for (const already of [2, 4, 6]) {
      if (grade === 1 && problem.target + already > 20) continue;
      pool.push(type === 'bridge' ? {
        ...problem, variant: 'repair', skill: 'Find the missing length',
        prompt: `Repair the river crossing. It must be ${problem.target + already} units long; ${already} units are already safe. Build only the missing section.`,
        hint: `Subtract the ${already} safe units from ${problem.target + already}. The empty track is the section you need to repair.`,
      } : {
        ...problem, variant: 'remaining-payment', price: problem.target + already, paid: already, skill: 'Find the remaining payment',
        prompt: `A fruit basket costs RM${problem.target + already}. You have already paid RM${already}. Put only the money still needed in the tray.`,
        hint: `Subtract the RM${already} already paid from the RM${problem.target + already} price.`,
      });
    }
  }
  // Numeric targets and contexts both vary. Pick without replacement each visit.
  const result = [];
  while (result.length < 3) {
    const problem = pool[Math.min(pool.length - 1, Math.max(0, Math.floor(rng() * pool.length)))];
    result.push(problem);
    pool = pool.filter(candidate => candidate.target !== problem.target);
  }
  return result;
}

function pairs(aMin, aMax, bMin, bMax) {
  return Array.from({length: aMax - aMin + 1}, (_, i) =>
    Array.from({length: bMax - bMin + 1}, (_, j) => [aMin + i, bMin + j])).flat();
}
...
export function startMission(container, { type, grade, onComplete, onExit, speak }) {
  ...
  function render() {
    ...
    if (type === 'bridge') {
      const targetLabel = problem.variant === 'repair' && !solved ? 'missing section' : problem.targetCm && !solved ? `${problem.targetCm} cm` : `${lengthLabel(problem, problem.target)} ${problem.unit || 'UNIT'}`;
      scene.append(element('div', 'mh-mission-scene-label', problem.variant === 'repair' && !solved ? 'BUILD THE MISSING SECTION' : `${targetLabel} CROSSING`));
      ...
      if (problem.paid) scene.append(element('p', 'mh-mission-grouping', `Already paid ${money(problem, problem.paid)} · Add only the remaining payment`));
